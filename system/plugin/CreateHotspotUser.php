...
function CreateHostspotUser()
{
    ...
    $phone = htmlspecialchars($postData['phone_number']);
    $planId = intval($postData['plan_id']);
    $routerId = intval($postData['router_id']);
    $mac_address = $postData['mac_address'];
    $macs = array("D4:CA:6D:C0:E3:10");
    ...
    $searchName = $mac_address;
    if (searchName($macs, $searchName)) {
        echo json_encode(['status' => 'error', 'code' => 405, 'message' => 'This device has been blocked from accessing this service, please contact service provider']);
        exit;
    }
...
function InitiateStkpush($phone, $planId, $routerId, $mac_address)
{
    ...
    if ($gateway == "MpesatillStk") {
        ...
        $url = "https://isp.speedcomwifi.co.ke/plugin/initiatebankstk";
