...
register_menu(" PPPoE Monitor", true, "pppoe_monitor_router_menu", 'CUSTOMERS');
...
function pppoe_monitor_router_delete_ppp_user()
{
    ...
    try {
        ...
        //$request = new RouterOS\Request('/ppp/secret set [find name=user1] disabled=yes');
        $request->setArgument('.id', $id); // Gunakan .id yang sesuai
        $client->sendSync($request);
        
        header('Content-Type: application/json');
        echo json_encode(['success' => true, 'message' => 'PPPoE user successfully suspended.']);
    } catch (Exception $e) {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'Failed to suspended PPPoE user: ' . $e->getMessage()]);
    }
}
...
function pppoe_monitor_router_set_disabled()
{
    global $routes;
    $router = $routes['2'];
    ...
    try {
        ...
        $request = new RouterOS\Request('/ppp/secret/set');
        $request->setArgument('.id', $pppUser->getProperty('.id'));
        $request->setArgument('disabled', 'yes');
        $client->sendSync($request);
        ...
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'Failed to Suspended User: ' . $e->getMessage()]);
    }
}
